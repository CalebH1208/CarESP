...
dataContainer CreateDataContainer(int length){
    ...
    char namesArr[length][30] = {"source\0","MPH\0","RPM\0","Voltage\0","Water Temperature\0","Oil Temperature\0","Oil Pressure\0","Fuel Pressure\0","Pitot Left\0","Pitot Right\0",
    "Pitot Center\0","Manifold Absolute Pressure\0","Lambda\0","Gear Position\0", "Shift Request\0","Neutral Sensor\0","Steering Angle\0","Lat Load\0","Long Load\0","Brake Pressure Front\0",
    "Brake Pressure Rear\0","Damper Pot FL\0","Damper Pot FR\0","Damper pot RL\0","Damper Pot RR\0","Throttle Position\0"};
    float convArr[length] = {1,.160934,1,.1,.1,.1,0.0145038,1,1,1,1,.0145038,.01,1,1,1,.1,0.00980665012,0.00980665012,0.145038,0.145038,1,1,1,1,.1,10};
    char endUnit[length][10] = {"N/A","N/A","N/A","V","F","F","PSI","PSI","PSI","PSI","PSI","PSI","N/A","N/A","N/A","N/A","Degrees","G","G","PSI","PSI","MM","MM","MM","MM","Percent","Temp"};
    for(int i =0;i<length;i++){
    strcpy(ret.dataArray[i].name, namesArr[i]);
    ret.dataArray[i].conv = convArr[i];
    strcpy(ret.dataArray[i].endUnit, endUnit[i]);
    ret.dataArray[i].value = i;
    }
...
dataContainer ret;
    ret.length = length;
    ret.dataArray =(datapoint*) malloc(sizeof(datapoint)*length);
    char namesArr[length][30] = {"source\0","Front Brake Pressure\0","Rear Brake Pressure\0","Command Torque\0","MPH\0","Inv Gate Driver PCB Temp\0","A Inverter Gate Driver\0","B Inverter Gate Driver\0","C Inverter Gate Driver\0","Inverter Hot Spot Temp\0",
    "Inverter DC-Link Current\0","Coolant Flow\0","APPS Plausibility Flag\0","Inv Est Coolant Temp\0","Motor Temp\0","GLV voltage\0","Inverter temp\0","RPM\0","Tractive sys Voltage\0","Highest Battery Temp\0",
    "Average Battery Temp\0","Total Pack Voltage\0","Accumulator Current\0","Water Temp\0"};
...
int updateArrayEV(twai_message_t message,dataContainer* DataC){
    switch(message.identifier){
      ...
      //case 0x702:
        ...
        // return 0;
      case 0x703:
        DataC->dataArray[14].value = (message.data[1] << 8) | message.data[2];
        DataC->dataArray[15].value = message.data[3];
        DataC->dataArray[11].value = (message.data[3] << 8) | message.data[4];
        DataC->dataArray[12].value = message.data[7];
        return 0;
      case 0x0A0:
        DataC->dataArray[6].value = (message.data[1] << 8) | message.data[0];
        DataC->dataArray[7].value = (message.data[3] << 8) | message.data[2];
        DataC->dataArray[8].value = (message.data[5] << 8) | message.data[4];
        DataC->dataArray[5].value = (message.data[7] << 8) | message.data[6];
        return 0;
      case 0x0A1:
        DataC->dataArray[16].value = (message.data[0] << 8) | message.data[1];
        DataC->dataArray[13].value = (message.data[2] << 8) | message.data[3];
        return 0;
      case 0x0A2:
        DataC->dataArray[9].value = (message.data[3] << 8) | message.data[2];
        return 0;
      case 0x0A5:
        DataC->dataArray[17].value = (message.data[3] << 8) | message.data[2];
        return 0;
      case 0x0A6:
        DataC->dataArray[10].value = (message.data[7] << 8) | message.data[6];
        return 0;
        case 0x0A7:
        DataC->dataArray[18].value = (message.data[1] << 8) | message.data[0];
        return 0;
      case 0x0AB:
        DataC->dataArray[23].value = (message.data[3] << 8) | message.data[2];
        DataC->dataArray[24].value = (message.data[1] << 8) | message.data[0];
        DataC->dataArray[25].value = (message.data[7] << 8) | message.data[6];
        DataC->dataArray[26].value = (message.data[5] << 8) | message.data[4];        
        return 0;
      case 0x6b1:
        DataC->dataArray[19].value = message.data[4];
        DataC->dataArray[20].value = message.data[6];
        return 0;
      case 0x6b0:
        DataC->dataArray[21].value = (message.data[2] << 8) | message.data[3];
        DataC->dataArray[22].value = (message.data[0] << 8) | message.data[1];
        return 0;
      default:
        return -2;
    }
...
int ec = radio.transmit(sendArray,DataArray.length*2);
return 0;
}
...
void DataLoggingSetup(dataContainer* data,int* Indexes1Hz,int Hz1Len,int* Indexes10Hz,int Hz10Len,int* Indexes100Hz,int Hz100Len,int* IndexesMaxHz,int HzMaxLen){
  ...
  while(1){
    ...
    // printf("%s",hey);
    ESP_LOGI("AHAHAH","HELLO WE SHOULD BE LOGGING HI");
...
if(i>(10000/(1000/params->loggingPeriodHz))){
  fclose(fptr);
